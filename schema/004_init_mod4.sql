...
(
    ...
    CONSTRAINT ORDER_store_id_fkey FOREIGN KEY (store_id)
        ...
        ON DELETE CASCADE,
    -- 狀態／型別／付款相關的 domain
    CONSTRAINT ORDER_status_chk CHECK (
        order_status IN ('placed', 'accepted', 'rejected', 'completed', 'cancelled')
    ),
    CONSTRAINT ORDER_type_chk CHECK (
        order_type IN ('pickup', 'delivery')
    ),
    CONSTRAINT ORDER_payment_status_chk CHECK (
        payment_status IN ('unpaid', 'paid', 'refunded')
    ),
    CONSTRAINT ORDER_payment_method_chk CHECK (
        payment_method IN ('cash', 'card', 'online')
    ),
    CONSTRAINT ORDER_total_price_chk CHECK (total_price >= 0)
);
...
(
    ...
    option_total_adjust int        NOT NULL,
...
CREATE TABLE IF NOT EXISTS public."ORDER_ITEM_RATING"
(
    order_item_id      bigint      NOT NULL,
